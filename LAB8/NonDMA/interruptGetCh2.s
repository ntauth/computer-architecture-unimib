...
la   $k0 ChDisponibile #decido che al ritorno (reg.EPC) SALTO al punto:
mtc0 $k0 $14           # 'ChDisponibile', DENTRO a 'GetCh'
...
#   e SE c'è un car. ricevuto disponibile in 'daTast' salto a 'ChDisponibile'
#   ALTRIMENTI salto a 'ciclo' per fare qualcosa nel frattempo

ciclo:
# faccio altro (qui stampo in un ciclo infinito il carattere 0)
        la      $a0, car0     	# stringa “0"
        li       $v0, 4        		# print string
        syscall
 
        li      $t0, 0x00080fff     		# costante grande
perdi_tempo:
        addi    $t0, $t0, -1             	# decrementa
        bnez    $t0, perdi_tempo       	# gira a vuoto
        b       ciclo          			# ciclo infinito 

ChDisponibile: #vi salto ANCHE direttamente dal gestore eccezioni
#>>>IMPLEMENTARE: prelevare e restituire car. in 'daTast' di gest. eccezioni
